...
- **修改密码**：已登录用户可自行修改密码（需验证当前密码）；配置 SMTP 后还支持「忘记密码」邮箱验证码找回。
- **管理员后台**：独立入口 `/admin`（与用户端分离），管理员可查看全站统计、用户列表、封禁/解封用户、删除用户、重置密码、提升/降级角色。首次启动自动创建默认管理员 `adminYYY` / `123456`，部署后请立即在后台右上角「修改密码」中更换。
...
- **反检测 / 仿真人行为**：内置多层反风控策略，让自动化请求看起来像真人操作：
  - 每个托管账号绑定固定的设备指纹（UA + 模拟 IP），登录后不再变化，账号间互不重复
  - IP 地址根据账号**注册所在省份**自动生成，模拟本地网络，不会触发异地登录
  - 听歌行为仿真：每轮少量歌曲（30-120 秒间隔）、播放时长随机波动、歌曲来源多样化
  - 调度随机抖动：用户之间执行时间不同步、周期间隔20-40 分钟随机
  - 可选 HTTP/SOCKS5 代理（`PROXY_URL`），支持住宅 IP 代理实现真实 IP 轮换
