...
一个开箱即用的网易云音乐「懒人自动化」面板：官方扫码登录后，后台按计划**全自动**为账号执行**每日签到**与**听歌打卡（刷听歌量，自动去重不重复）**，所有功能均可一键开关。专为部署到自己的服务器（如**宝塔面板**）长期托管而设计。
...
- **全自动调度**：内置 `node-cron` 定时器（默认每天 08:00，时区 Asia/Shanghai），按计划自动跑所有开启的任务，真正懒人托管。
...
6. **访问面板**：打开 `http://服务器IP:3000`（或你的域名）→ 点「添加账号」→ 用网易云 App 扫码 → 打开「自动签到 / 自动听歌打卡」开关 → 完成，后台会按计划自动运行。
...
| GET/POST | `/api/scheduler` | 查看 / 修改定时调度 |
